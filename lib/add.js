...
function readLogData() {
  try {
    var contents = fs.readFileSync(path.resolve(cwd, './package.json'), 'utf8').toString();
...
function addTopPage(name, type) {
  ...
  // 添加默认的log信息
  allNames = _.extend({}, config, toolUtil.generateNames(name), { type: type}, logData);
...
function addData(name) {
  ...
  if (fs.existsSync(apiMapFile)) {
    ...
    apiMapContent = fieInstance.fileRewrite({
      ...
      hook: '/* invoke */',
      insertLines: [
        '  ' + allNames.varName + ': [\'/data/' + allNames.fileName + '.json\', \'get\'],'
      ]
    });
